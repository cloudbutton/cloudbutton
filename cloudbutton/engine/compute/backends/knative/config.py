...
RUNTIME_NAME_DEFAULT = 'cloudbutton-knative'

BUILD_GIT_URL_DEFAULT = 'https://github.com/cloudbutton/cloudbutton'
...
FH_ZIP_LOCATION = os.path.join(os.getcwd(), 'cloudbutton_knative.zip')
...
kind: ServiceAccount
metadata:
  name: cloudbutton-build-pipeline
secrets:
- name: dockerhub-user-token
"""
...
spec:
  ...
  params:
    - name: revision
      value: master
    - name: url
      value: https://github.com/cloudbutton/cloudbutton
"""
...
kind: TaskRun
metadata:
  name: cloudbutton-runtime-from-git
spec:
  serviceAccountName: cloudbutton-build-pipeline
  taskRef:
    name: git-source-to-image
  inputs:
    resources:
    ...
    params:
      - name: pathToDockerFile
        value: cloudbutton/engine/compute/backends/knative/tekton/Dockerfile.python36
      - name: imageUrl
        value: docker.io/jsampe/cloudbutton-knative-v36
      - name: imageTag
        value: latest
"""
...
kind: Service
metadata:
  name: cloudbutton-runtime
  #namespace: default
spec:
  template:
    metadata:
      labels:
        type: cloudbutton-runtime
      #annotations:
